...
export async function promoteAuxMemberToLgn({ discordUser, context }: PromoteAuxMemberToLgnParams) {
    ...
    await discordUser.roles.set(
        [...finalRoleIds],
        'Promoted AUX to LGN from voice activity credits',
    );
    ...
    await deleteManyDivisionMembership({
        discordUserId: discordUser.id,
        divisionIds: [
            (await container.utilities.divisionCache.getByCode('AUX'))!.id,
        ]
    });
    ...
    await createManyDivisionMembership({
        discordUserId: discordUser.id,
        divisionIds: [
            (await container.utilities.divisionCache.getByCode('LGN'))!.id,
        ]
    });
}
