...
export async function handleJoinDivision(
    {
        userDbId,
        interaction,
    ...
    if (!guildMember) {
        ...
        interaction.editReply({ content: `There was an error processing your selection. Please contact a TECH member with the following: discordMessageId=${interaction.id} customButtonId=${interaction.customId}` });
        return;
    }
    ...
    if (existingSameKindRoleIds.length > 0) {
        ...
        await guildMember.roles.remove(
            existingSameKindRoleIds,
            `Replacing ${selectedDivision.kind} division role via button selection`,
        );
    }

    await guildMember.roles.add(
        selectedDivision.discordRoleId,
        `Joined ${selectedDivision.name} division via button selection`,
    );
    logger.info({
        userDbId,
        discordMessageId: interaction.id,
