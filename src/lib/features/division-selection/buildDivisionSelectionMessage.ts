		...
		if (
			...
			if (division.kind === DivisionKind.COMBAT) {
				...
				industrialEmbed.fields!.push({
					name: `<:${division.emojiName}:${division.emojiId}> **Division: ${division.name}**`,
					value: SELECTABLE_DIVISIONS.INDUSTRIAL[division.code as 'DRL' | 'SCR' | 'LOG' | 'TRD' | 'ARC'].join('\n'),
					inline: false
				});
				industrialEmbed.fields!.push({
					name: '\u200B',
...
function toRowsByGroup(combatButtons: APIButtonComponent[], industrialButtons: APIButtonComponent[]): APIActionRowComponent<APIButtonComponent>[] {
