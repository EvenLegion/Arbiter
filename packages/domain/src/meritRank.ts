...
const MERIT_RANK_SYMBOL_RANGES: ReadonlyArray<{
	minLevel: number;
	maxLevel: number;
	codePointOffset: number;
}> = [
	{ minLevel: 1, maxLevel: 20, codePointOffset: 9311 },
	{ minLevel: 21, maxLevel: 35, codePointOffset: 12860 },
	{ minLevel: 36, maxLevel: 50, codePointOffset: 12941 }
];
...
export function resolveMeritRankLevel(totalMerits: number): number | null {
	...
	return closestThresholdIndex === null ? null : (LEVELS_BY_THRESHOLD_INDEX[closestThresholdIndex] ?? null);
}
...
export function getMeritRankSymbol(level: number): string | null {
	...
	return matchingRange ? String.fromCodePoint(matchingRange.codePointOffset + level) : null;
}
...
export function resolveMeritRankProgress(totalMerits: number): MeritRankProgress {
	...
	const currentThreshold = currentLevel ? (LEVEL_TO_CUMULATIVE_MERITS.get(currentLevel) ?? 0) : 0;
	const nextLevel = currentLevel ? currentLevel + 1 : 1;
	const nextThreshold = LEVEL_TO_CUMULATIVE_MERITS.get(nextLevel) ?? null;
...
function findClosestThresholdIndexAtOrBelow(totalMerits: number): number | null {
	if (SORTED_CUMULATIVE_MERITS.length === 0 || totalMerits < SORTED_CUMULATIVE_MERITS[0]) return null;
