...
If a timeout, network interruption, or unavailable service makes a registry mutation outcome uncertain, the portal retries only operations that safely converge, then refreshes the registry. It never retries mint automatically because a committed mint whose response was lost has an unrecoverable secret. Instead it refreshes credential metadata and tells the operator to revoke any newly visible label before minting another credential. Revocation preserves the API's first authoritative actor and timestamp; after an interrupted revoke, the portal refreshes metadata to reconcile the result.
