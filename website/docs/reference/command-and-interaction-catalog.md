...
| ----------------------------- | ------------------------------------------------------- | -------------------------------------------------------------------------------- | ------------------------------------ | ----------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| division selection buttons    | `src/interaction-handlers/divisionSelectionButton.ts`   | `divisionSelectionCustomId.ts` and division-selection custom ids                 | `handleDivisionSelectionButton.ts`   | `divisionSelectionService.ts`                                           | [Division And Membership](/features/division-and-membership)             |
| event start buttons           | `src/interaction-handlers/eventStartButtons.ts`         | `eventStartButtonCustomId.ts` and event-start button custom ids                  | `handleEventStartButton.ts`          | `src/lib/services/event-lifecycle/index.ts`                             | [Event Session Lifecycle](/features/event-session-lifecycle)             |
| event review buttons          | `src/interaction-handlers/eventReviewButtons.ts`        | `eventReviewButtonProtocol.ts` and event-review paging, decision, and submit ids | `handleEventReviewButton.ts`         | `eventReviewService.ts` and `src/lib/services/event-lifecycle/index.ts` | [Event Review And Finalization](/features/event-review-and-finalization) |
| merit list buttons            | `src/interaction-handlers/meritListButtons.ts`          | `meritListButtonCustomId.ts` and merit-list page custom ids                      | `handleMeritList.ts`                 | `meritReadService.ts`                                                   | [Merit System](/features/merit-system)                                   |
| name-change review buttons    | `src/interaction-handlers/nameChangeReviewButtons.ts`   | `nameChangeReviewCustomId.ts` and name-change review button custom ids           | `handleNameChangeReviewButton.ts`    | `name-change` service functions                                         | [Name Change Workflow](/features/name-change-workflow)                   |
| name-change review edit modal | `src/interaction-handlers/nameChangeReviewEditModal.ts` | `nameChangeReviewCustomId.ts` and name-change review edit modal ids              | `handleNameChangeReviewEditModal.ts` | `editPendingNameChangeRequest(...)`                                     | [Name Change Workflow](/features/name-change-workflow)                   |
